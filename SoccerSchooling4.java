...
// andrew, mitul
// Class with main
public class SoccerSchooling4
{
	public SoccerSchooling4()
	{
	}
	
	public static void main(String[] args)
	{
		SoccerSchooling4 game = new SoccerSchooling4();
		game.run();
	}
...
{
	private JLabel[] names; // array of names
	private JLabel[] scores; // array of scores
	...
	public HighScoresGrid(AddCards acIn)
	{
		...
		for (int i = 0; i < names.length; i++)
		{
			...
			names[i].setFont(labelFont);
			scores[i].setFont(labelFont);
			add(names[i]);
			add(scores[i]);
		}
	}
	...
	public void setScores()
	{
		...
		while(readIn.hasNext() && place < 5)
		{
			...
			if (!line.equals(""))
			{
				names[place].setText(line.substring(0, line.lastIndexOf("-")).trim());
				scores[place].setText(line.substring(line.lastIndexOf("-") + 1).trim());
				place++;
			}
		}
		for (int i = 0; i < names.length; i++)
...
class ProbQuestions
{
	...
	private String[] type; // 3 different prompts
	...
	public ProbQuestions(GenerateQuestions gqIn)
	{
		...
		type = new String[] {"How many words of", "How many different ways", "What is the prob"};
	...
	// mitul
	// uses prompt 3 to generate a random question
	public double getCriteria3()
	{
		word = word.toUpperCase();
		int howMany = (int)(Math.random()*(word.length() - 2) + 1);
		char atI2 ='1';
		// adds the amount of each letter into an array
		for (int i = 0; i < word.length(); i++)
		{
			atI2 = word.charAt(i);
			repeats[atI2 - 65]++;
		}
		// so the same index is nto asked twice
		int[] askedIndices = new int[howMany];
		for (int k = 0; k< askedIndices.length; k++)
		{
			askedIndices[k] = -1;
		}
		double answer = 1;
		int whichType = (int)(Math.random()*2);
		int nextIndex = 0;
		finalQuestion = type[2] + "ability of getting a(n) ";
		nextIndex = (int)(Math.random()*word.length());
		if (whichType == 0)
		{
			// checks if the index has been asked
			for (int j = 0; j < howMany; j++)
			{
				for (int o = 0; o < askedIndices.length; o++)
				{
					if (nextIndex == askedIndices[o])
					{
						nextIndex = (int)(Math.random()*word.length());
						o = 0;
					}
				...
				askedIndices[j] = nextIndex;
				atI2 = word.charAt(nextIndex);
				answer *= (double)repeats[atI2 - 65]/(double)(word.length() - j);
				if (j < howMany - 1)
					finalQuestion += "'" + (char)(atI2 + 32) + "', then a(n) ";
				else 
					finalQuestion += "'" + (char)(atI2 + 32) + "' ";
				repeats[atI2 - 65]--;
			}
		}
		else
		{
			...
			{
				for (int n = 0; n < askedIndices.length; n++)
				{
					if (nextIndex == askedIndices[n])
					{
						nextIndex = (int)(Math.random()*word.length());
						n = 0;
					}
				}
				askedIndices[m] = nextIndex;
				atI2 = word.charAt(nextIndex);
				answer += (double)(repeats[atI2 - 65])/(double)(word.length() );
				if (m == howMany - 1)
					finalQuestion += "'" + (char)(atI2 + 32) + "' ";
				else if (m == howMany - 2)
					finalQuestion += "'" + (char)(atI2 + 32) + "', or a(n) ";
				else
					finalQuestion += "'" + (char)(atI2 + 32) + "', a(n) ";
			}
		}
		finalQuestion += "by randomely selecting letters in the word '" + word.toLowerCase() +
			"' without repetition?";
		return answer;
	}
...
class StatsQuestions 
{
	...
	public void simpQuest2()
	{
		...
		File scenarios2 = new File("sampling.txt");
		try
		{
	...
	public void diffQuest2()
	{
		...
		Scanner readIn = null;
		File scenarios = new File("studies.txt");
		try
		{
